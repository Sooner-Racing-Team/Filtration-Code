...
// https://github.com/denyssene/SimpleKalmanFilter (found a klamn filter library, we could compare that to moving average)
...
//for loop in the x-axis, average of 10 accel-x values
  ...
  for(int i=0; i<=9; i++) {
    a_avg_x = a_avg_x + a_treadmill_x[i];
  }
  a_avg_x = a_avg_x / 10;
...
//taking the average of 5 averaged accel-x (revise; does not work very well)
  ...
  for(int i=0; i<=4; i++) {
    a_avg2_x = a_avg2_x + avg_treadmill_x[i];
  }
...
//for loop in the y-axis
  ...
  for(int i=0; i<=9; i++) {
    a_avg_y = a_avg_y + a_treadmill_y[i];
  }
  a_avg_y = a_avg_y / 10;
